...
Page({
  ...
  data: {
    categories: [],
    activeCategory: 0,
    categorySelected: {  //当前选择的左边列表的分类对象：上装/裤装/...
      name: '',
      id: ''
    },   
    currentGoods: [],    //当前选中分类的所有商品
    onLoadStatus: true,
    scrolltop: 0,
  ...
  // 根据分类ID筛选所有的商品currentGoods:WXAPI:doosv2(categoryId)
  async getGoodsList() {
  ...
  // 左边分类被点击事件,获取currentGoods
  async onCategoryClick(e) {
    const idx = e.target.dataset.idx
    // 如果就是当前被激活的分类，点击的话返回到顶部
  ...
  // 搜索框确定事件，跳转至goods/list页面，显示搜索结果页面
  bindconfirm(e) {
    this.setData({
      inputVal: e.detail
    })
